...
from hashUtil import hash_string_256, hash_block
...
def loadData():
    with open('blockchain.txt', mode='r') as f:
        ...
        # fileContent = pickle.loads(f.readlines())

        global blockchain
        global openTransactions
        # blockchain = fileContent['chain']
        # openTransactions = fileContent['ot']
        # # Old logic to save data with use of json 
...
def proof_of_work():
    ...
    while not valid_proof(openTransactions, last_hash, proof):
        proof += 1
    return proof
...
def get_balance(participant):
    ...
    open_tx_sender = [tx['amount']
                      for tx in openTransactions if tx['sender'] == participant]
...
def verify_transactions():
    ...
    return all([verify_transaction(tx) for tx in openTransactions])
...
while waiting_for_input:
    ...
    if not verify_chain():
        ...
        break
    print('Balance of {}: {:6.2f}'.format('Dominik', get_balance('Dominik')))
else:
    print('User left!')
